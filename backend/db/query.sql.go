// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: query.sql

package db

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

const checkInGame = `-- name: CheckInGame :exec
UPDATE transactions
SET checkin_timestamp = now()
WHERE id = $1 AND checkin_timestamp IS NULL
`

func (q *Queries) CheckInGame(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, checkInGame, id)
	return err
}

const checkOutGame = `-- name: CheckOutGame :one
INSERT INTO transactions (game_id, patron_id) VALUES ($1, $2)
RETURNING id, game_id, patron_id, checkout_timestamp, checkin_timestamp
`

type CheckOutGameParams struct {
	GameID   pgtype.UUID
	PatronID pgtype.UUID
}

func (q *Queries) CheckOutGame(ctx context.Context, arg CheckOutGameParams) (Transaction, error) {
	row := q.db.QueryRow(ctx, checkOutGame, arg.GameID, arg.PatronID)
	var i Transaction
	err := row.Scan(
		&i.ID,
		&i.GameID,
		&i.PatronID,
		&i.CheckoutTimestamp,
		&i.CheckinTimestamp,
	)
	return i, err
}

const createGame = `-- name: CreateGame :one
INSERT INTO games ( title, sanitized_title, barcode ) VALUES ( $1, $2, $3 )
RETURNING id, title, display_title, sanitized_title, created_at, deleted_at, barcode
`

type CreateGameParams struct {
	Title          string
	SanitizedTitle string
	Barcode        pgtype.Text
}

func (q *Queries) CreateGame(ctx context.Context, arg CreateGameParams) (Game, error) {
	row := q.db.QueryRow(ctx, createGame, arg.Title, arg.SanitizedTitle, arg.Barcode)
	var i Game
	err := row.Scan(
		&i.ID,
		&i.Title,
		&i.DisplayTitle,
		&i.SanitizedTitle,
		&i.CreatedAt,
		&i.DeletedAt,
		&i.Barcode,
	)
	return i, err
}

const createPatron = `-- name: CreatePatron :one
INSERT INTO patrons ( full_name, barcode ) VALUES ( $1, $2 )
RETURNING id, full_name, created_at, deleted_at, barcode
`

type CreatePatronParams struct {
	FullName string
	Barcode  pgtype.Text
}

func (q *Queries) CreatePatron(ctx context.Context, arg CreatePatronParams) (Patron, error) {
	row := q.db.QueryRow(ctx, createPatron, arg.FullName, arg.Barcode)
	var i Patron
	err := row.Scan(
		&i.ID,
		&i.FullName,
		&i.CreatedAt,
		&i.DeletedAt,
		&i.Barcode,
	)
	return i, err
}

const createPlayToWinEntry = `-- name: CreatePlayToWinEntry :one
INSERT INTO play_to_win_entries (ptw_session_id, ptw_group_id, entrant_name, entrant_unique_id) VALUES ($1, $2, $3, $4)
RETURNING id, ptw_session_id, ptw_group_id, entrant_name, entrant_unique_id, created_at, deleted_at, deletion_reason, deletion_reason_comment
`

type CreatePlayToWinEntryParams struct {
	PtwSessionID    pgtype.UUID
	PtwGroupID      pgtype.UUID
	EntrantName     string
	EntrantUniqueID string
}

func (q *Queries) CreatePlayToWinEntry(ctx context.Context, arg CreatePlayToWinEntryParams) (PlayToWinEntry, error) {
	row := q.db.QueryRow(ctx, createPlayToWinEntry,
		arg.PtwSessionID,
		arg.PtwGroupID,
		arg.EntrantName,
		arg.EntrantUniqueID,
	)
	var i PlayToWinEntry
	err := row.Scan(
		&i.ID,
		&i.PtwSessionID,
		&i.PtwGroupID,
		&i.EntrantName,
		&i.EntrantUniqueID,
		&i.CreatedAt,
		&i.DeletedAt,
		&i.DeletionReason,
		&i.DeletionReasonComment,
	)
	return i, err
}

const createPlayToWinGame = `-- name: CreatePlayToWinGame :one
INSERT INTO play_to_win_games (game_id, ptw_group_id) VALUES ($1, $2)
RETURNING id, game_id, ptw_group_id, winner_id, created_at, deleted_at, deletion_reason, deletion_reason_comment
`

type CreatePlayToWinGameParams struct {
	GameID     pgtype.UUID
	PtwGroupID pgtype.UUID
}

func (q *Queries) CreatePlayToWinGame(ctx context.Context, arg CreatePlayToWinGameParams) (PlayToWinGame, error) {
	row := q.db.QueryRow(ctx, createPlayToWinGame, arg.GameID, arg.PtwGroupID)
	var i PlayToWinGame
	err := row.Scan(
		&i.ID,
		&i.GameID,
		&i.PtwGroupID,
		&i.WinnerID,
		&i.CreatedAt,
		&i.DeletedAt,
		&i.DeletionReason,
		&i.DeletionReasonComment,
	)
	return i, err
}

const createPlayToWinGroup = `-- name: CreatePlayToWinGroup :one
INSERT INTO play_to_win_groups (name) VALUES ($1)
RETURNING id, name, created_at, deleted_at
`

func (q *Queries) CreatePlayToWinGroup(ctx context.Context, name string) (PlayToWinGroup, error) {
	row := q.db.QueryRow(ctx, createPlayToWinGroup, name)
	var i PlayToWinGroup
	err := row.Scan(
		&i.ID,
		&i.Name,
		&i.CreatedAt,
		&i.DeletedAt,
	)
	return i, err
}

const createPlayToWinSession = `-- name: CreatePlayToWinSession :one
INSERT INTO play_to_win_sessions (ptw_group_id, playtime_minutes) VALUES ($1, $2)
RETURNING id, ptw_group_id, playtime_minutes, created_at, deleted_at, deletion_reason, deletion_reason_comment
`

type CreatePlayToWinSessionParams struct {
	PtwGroupID      pgtype.UUID
	PlaytimeMinutes pgtype.Int4
}

func (q *Queries) CreatePlayToWinSession(ctx context.Context, arg CreatePlayToWinSessionParams) (PlayToWinSession, error) {
	row := q.db.QueryRow(ctx, createPlayToWinSession, arg.PtwGroupID, arg.PlaytimeMinutes)
	var i PlayToWinSession
	err := row.Scan(
		&i.ID,
		&i.PtwGroupID,
		&i.PlaytimeMinutes,
		&i.CreatedAt,
		&i.DeletedAt,
		&i.DeletionReason,
		&i.DeletionReasonComment,
	)
	return i, err
}

const deleteGame = `-- name: DeleteGame :exec
UPDATE games
SET deleted_at = now(),
    barcode = NULL
WHERE deleted_at IS NULL AND id = $1
`

func (q *Queries) DeleteGame(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteGame, id)
	return err
}

const deletePatron = `-- name: DeletePatron :exec
UPDATE patrons
SET deleted_at = now(),
    barcode = NULL,
    full_name = 'Deleted Patron'
WHERE deleted_at IS NULL AND id = $1
`

func (q *Queries) DeletePatron(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deletePatron, id)
	return err
}

const deletePlayToWinEntry = `-- name: DeletePlayToWinEntry :exec
UPDATE play_to_win_entries
SET deleted_at = now(),
    deletion_reason = $2,
    deletion_reason_comment = $3
WHERE deleted_at IS NULL AND id = $1
`

type DeletePlayToWinEntryParams struct {
	ID                    pgtype.UUID
	DeletionReason        NullPlayToWinEntryDeletionType
	DeletionReasonComment pgtype.Text
}

func (q *Queries) DeletePlayToWinEntry(ctx context.Context, arg DeletePlayToWinEntryParams) error {
	_, err := q.db.Exec(ctx, deletePlayToWinEntry, arg.ID, arg.DeletionReason, arg.DeletionReasonComment)
	return err
}

const deletePlayToWinGame = `-- name: DeletePlayToWinGame :exec
UPDATE play_to_win_games
SET deleted_at = now(),
    deletion_reason = $2,
    deletion_reason_comment = $3
WHERE deleted_at IS NULL AND game_id = $1
`

type DeletePlayToWinGameParams struct {
	GameID                pgtype.UUID
	DeletionReason        NullPlayToWinGameDeletionType
	DeletionReasonComment pgtype.Text
}

func (q *Queries) DeletePlayToWinGame(ctx context.Context, arg DeletePlayToWinGameParams) error {
	_, err := q.db.Exec(ctx, deletePlayToWinGame, arg.GameID, arg.DeletionReason, arg.DeletionReasonComment)
	return err
}

const deletePlayToWinGameByPlayToWinId = `-- name: DeletePlayToWinGameByPlayToWinId :exec
UPDATE play_to_win_games
SET deleted_at = now(),
    deletion_reason = $2,
    deletion_reason_comment = $3
WHERE deleted_at IS NULL AND id = $1
`

type DeletePlayToWinGameByPlayToWinIdParams struct {
	ID                    pgtype.UUID
	DeletionReason        NullPlayToWinGameDeletionType
	DeletionReasonComment pgtype.Text
}

func (q *Queries) DeletePlayToWinGameByPlayToWinId(ctx context.Context, arg DeletePlayToWinGameByPlayToWinIdParams) error {
	_, err := q.db.Exec(ctx, deletePlayToWinGameByPlayToWinId, arg.ID, arg.DeletionReason, arg.DeletionReasonComment)
	return err
}

const deletePlayToWinSession = `-- name: DeletePlayToWinSession :exec
UPDATE play_to_win_sessions
SET deleted_at = now(),
    deletion_reason = $2,
    deletion_reason_comment = $3
WHERE deleted_at IS NULL AND id = $1
`

type DeletePlayToWinSessionParams struct {
	ID                    pgtype.UUID
	DeletionReason        NullPlayToWinSessionDeletionType
	DeletionReasonComment pgtype.Text
}

func (q *Queries) DeletePlayToWinSession(ctx context.Context, arg DeletePlayToWinSessionParams) error {
	_, err := q.db.Exec(ctx, deletePlayToWinSession, arg.ID, arg.DeletionReason, arg.DeletionReasonComment)
	return err
}

const editGame = `-- name: EditGame :exec
UPDATE games
SET display_title = $2,
    sanitized_title = $3,
    barcode = $4
WHERE id = $1
`

type EditGameParams struct {
	ID             pgtype.UUID
	DisplayTitle   pgtype.Text
	SanitizedTitle string
	Barcode        pgtype.Text
}

func (q *Queries) EditGame(ctx context.Context, arg EditGameParams) error {
	_, err := q.db.Exec(ctx, editGame,
		arg.ID,
		arg.DisplayTitle,
		arg.SanitizedTitle,
		arg.Barcode,
	)
	return err
}

const editPatron = `-- name: EditPatron :exec
UPDATE patrons
SET full_name = $2,
    barcode = $3
WHERE id = $1
`

type EditPatronParams struct {
	ID       pgtype.UUID
	FullName string
	Barcode  pgtype.Text
}

func (q *Queries) EditPatron(ctx context.Context, arg EditPatronParams) error {
	_, err := q.db.Exec(ctx, editPatron, arg.ID, arg.FullName, arg.Barcode)
	return err
}

const getGame = `-- name: GetGame :one
SELECT id, display_title, title, sanitized_title, barcode, play_to_win_game_id, created_at
FROM vw_library_games
WHERE id = $1
`

func (q *Queries) GetGame(ctx context.Context, id pgtype.UUID) (VwLibraryGame, error) {
	row := q.db.QueryRow(ctx, getGame, id)
	var i VwLibraryGame
	err := row.Scan(
		&i.ID,
		&i.DisplayTitle,
		&i.Title,
		&i.SanitizedTitle,
		&i.Barcode,
		&i.PlayToWinGameID,
		&i.CreatedAt,
	)
	return i, err
}

const getGameByBarcode = `-- name: GetGameByBarcode :many
SELECT id, display_title, title, sanitized_title, barcode, play_to_win_game_id, created_at
FROM vw_library_games
WHERE barcode = $1
`

func (q *Queries) GetGameByBarcode(ctx context.Context, barcode pgtype.Text) ([]VwLibraryGame, error) {
	rows, err := q.db.Query(ctx, getGameByBarcode, barcode)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwLibraryGame
	for rows.Next() {
		var i VwLibraryGame
		if err := rows.Scan(
			&i.ID,
			&i.DisplayTitle,
			&i.Title,
			&i.SanitizedTitle,
			&i.Barcode,
			&i.PlayToWinGameID,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getGameStatus = `-- name: GetGameStatus :one
SELECT game_id, game_title, game_barcode, sanitized_title, patron_id, patron_full_name, patron_barcode, transaction_id, checkout_timestamp, checkin_timestamp, checked_out, ptw_game_id
FROM vw_game_status
WHERE game_id = $1
`

func (q *Queries) GetGameStatus(ctx context.Context, gameID pgtype.UUID) (VwGameStatus, error) {
	row := q.db.QueryRow(ctx, getGameStatus, gameID)
	var i VwGameStatus
	err := row.Scan(
		&i.GameID,
		&i.GameTitle,
		&i.GameBarcode,
		&i.SanitizedTitle,
		&i.PatronID,
		&i.PatronFullName,
		&i.PatronBarcode,
		&i.TransactionID,
		&i.CheckoutTimestamp,
		&i.CheckinTimestamp,
		&i.CheckedOut,
		&i.PtwGameID,
	)
	return i, err
}

const getPatron = `-- name: GetPatron :one
SELECT id, full_name, barcode, created_at
FROM vw_library_patrons
WHERE id = $1
`

func (q *Queries) GetPatron(ctx context.Context, id pgtype.UUID) (VwLibraryPatron, error) {
	row := q.db.QueryRow(ctx, getPatron, id)
	var i VwLibraryPatron
	err := row.Scan(
		&i.ID,
		&i.FullName,
		&i.Barcode,
		&i.CreatedAt,
	)
	return i, err
}

const getPatronByBarcode = `-- name: GetPatronByBarcode :one
SELECT id, full_name, barcode, created_at
FROM vw_library_patrons
WHERE barcode = $1
`

func (q *Queries) GetPatronByBarcode(ctx context.Context, barcode pgtype.Text) (VwLibraryPatron, error) {
	row := q.db.QueryRow(ctx, getPatronByBarcode, barcode)
	var i VwLibraryPatron
	err := row.Scan(
		&i.ID,
		&i.FullName,
		&i.Barcode,
		&i.CreatedAt,
	)
	return i, err
}

const getPlayToWinEntriesByGroupId = `-- name: GetPlayToWinEntriesByGroupId :many
SELECT id, ptw_session_id, ptw_group_id, entrant_name, entrant_unique_id, created_at
FROM vw_play_to_win_entries
WHERE ptw_group_id = $1
`

func (q *Queries) GetPlayToWinEntriesByGroupId(ctx context.Context, ptwGroupID pgtype.UUID) ([]VwPlayToWinEntry, error) {
	rows, err := q.db.Query(ctx, getPlayToWinEntriesByGroupId, ptwGroupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwPlayToWinEntry
	for rows.Next() {
		var i VwPlayToWinEntry
		if err := rows.Scan(
			&i.ID,
			&i.PtwSessionID,
			&i.PtwGroupID,
			&i.EntrantName,
			&i.EntrantUniqueID,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getPlayToWinEntriesByPlayToWinGameId = `-- name: GetPlayToWinEntriesByPlayToWinGameId :many
SELECT e.id, e.ptw_session_id, e.ptw_group_id, e.entrant_name, e.entrant_unique_id, e.created_at
FROM vw_play_to_win_entries e
LEFT JOIN play_to_win_games g ON e.ptw_group_id = g.ptw_group_id
WHERE g.id = $1
`

func (q *Queries) GetPlayToWinEntriesByPlayToWinGameId(ctx context.Context, id pgtype.UUID) ([]VwPlayToWinEntry, error) {
	rows, err := q.db.Query(ctx, getPlayToWinEntriesByPlayToWinGameId, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwPlayToWinEntry
	for rows.Next() {
		var i VwPlayToWinEntry
		if err := rows.Scan(
			&i.ID,
			&i.PtwSessionID,
			&i.PtwGroupID,
			&i.EntrantName,
			&i.EntrantUniqueID,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getPlayToWinGameByLibraryGameId = `-- name: GetPlayToWinGameByLibraryGameId :one
SELECT id, game_id, ptw_group_id, group_name, created_at, winner_id
FROM vw_play_to_win_games
WHERE game_id = $1
`

func (q *Queries) GetPlayToWinGameByLibraryGameId(ctx context.Context, gameID pgtype.UUID) (VwPlayToWinGame, error) {
	row := q.db.QueryRow(ctx, getPlayToWinGameByLibraryGameId, gameID)
	var i VwPlayToWinGame
	err := row.Scan(
		&i.ID,
		&i.GameID,
		&i.PtwGroupID,
		&i.GroupName,
		&i.CreatedAt,
		&i.WinnerID,
	)
	return i, err
}

const getPlayToWinGameOverview = `-- name: GetPlayToWinGameOverview :one
SELECT ptw_game_id, game_id, ptw_group_id, game_title, sanitized_title, created_at, winner_id, winner_name, winner_unique_id
FROM vw_play_to_win_game_overview
WHERE ptw_game_id = $1
`

func (q *Queries) GetPlayToWinGameOverview(ctx context.Context, ptwGameID pgtype.UUID) (VwPlayToWinGameOverview, error) {
	row := q.db.QueryRow(ctx, getPlayToWinGameOverview, ptwGameID)
	var i VwPlayToWinGameOverview
	err := row.Scan(
		&i.PtwGameID,
		&i.GameID,
		&i.PtwGroupID,
		&i.GameTitle,
		&i.SanitizedTitle,
		&i.CreatedAt,
		&i.WinnerID,
		&i.WinnerName,
		&i.WinnerUniqueID,
	)
	return i, err
}

const getPlayToWinGroup = `-- name: GetPlayToWinGroup :one
SELECT id, name, created_at
FROM vw_play_to_win_groups
WHERE id = $1
`

func (q *Queries) GetPlayToWinGroup(ctx context.Context, id pgtype.UUID) (VwPlayToWinGroup, error) {
	row := q.db.QueryRow(ctx, getPlayToWinGroup, id)
	var i VwPlayToWinGroup
	err := row.Scan(&i.ID, &i.Name, &i.CreatedAt)
	return i, err
}

const getPlayToWinGroupByName = `-- name: GetPlayToWinGroupByName :one
SELECT id, name, created_at
FROM vw_play_to_win_groups
WHERE name ILIKE $1
`

func (q *Queries) GetPlayToWinGroupByName(ctx context.Context, name string) (VwPlayToWinGroup, error) {
	row := q.db.QueryRow(ctx, getPlayToWinGroupByName, name)
	var i VwPlayToWinGroup
	err := row.Scan(&i.ID, &i.Name, &i.CreatedAt)
	return i, err
}

const getPlayToWinGroupByPlayToWinGameId = `-- name: GetPlayToWinGroupByPlayToWinGameId :one
SELECT gr.id, gr.name, gr.created_at
FROM vw_play_to_win_groups AS gr
LEFT JOIN play_to_win_games AS ga ON gr.id = ga.ptw_group_id
WHERE ga.id = $1
`

func (q *Queries) GetPlayToWinGroupByPlayToWinGameId(ctx context.Context, id pgtype.UUID) (VwPlayToWinGroup, error) {
	row := q.db.QueryRow(ctx, getPlayToWinGroupByPlayToWinGameId, id)
	var i VwPlayToWinGroup
	err := row.Scan(&i.ID, &i.Name, &i.CreatedAt)
	return i, err
}

const getPlayToWinSessionsByGroupId = `-- name: GetPlayToWinSessionsByGroupId :many
SELECT id, ptw_group_id, playtime_minutes, created_at
FROM vw_play_to_win_sessions
WHERE ptw_group_id = $1
`

func (q *Queries) GetPlayToWinSessionsByGroupId(ctx context.Context, ptwGroupID pgtype.UUID) ([]VwPlayToWinSession, error) {
	rows, err := q.db.Query(ctx, getPlayToWinSessionsByGroupId, ptwGroupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwPlayToWinSession
	for rows.Next() {
		var i VwPlayToWinSession
		if err := rows.Scan(
			&i.ID,
			&i.PtwGroupID,
			&i.PlaytimeMinutes,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listCheckedOutGames = `-- name: ListCheckedOutGames :many
SELECT game_id, game_title, game_barcode, sanitized_title, patron_id, patron_full_name, patron_barcode, transaction_id, checkout_timestamp, checkin_timestamp, checked_out, ptw_game_id
FROM vw_game_status
WHERE checkin_timestamp IS NULL AND checkout_timestamp IS NOT NULL
ORDER BY sanitized_title
LIMIT $1 OFFSET $2
`

type ListCheckedOutGamesParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListCheckedOutGames(ctx context.Context, arg ListCheckedOutGamesParams) ([]VwGameStatus, error) {
	rows, err := q.db.Query(ctx, listCheckedOutGames, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwGameStatus
	for rows.Next() {
		var i VwGameStatus
		if err := rows.Scan(
			&i.GameID,
			&i.GameTitle,
			&i.GameBarcode,
			&i.SanitizedTitle,
			&i.PatronID,
			&i.PatronFullName,
			&i.PatronBarcode,
			&i.TransactionID,
			&i.CheckoutTimestamp,
			&i.CheckinTimestamp,
			&i.CheckedOut,
			&i.PtwGameID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listGames = `-- name: ListGames :many
SELECT id, display_title, title, sanitized_title, barcode, play_to_win_game_id, created_at
FROM vw_library_games
ORDER BY sanitized_title
LIMIT $1 OFFSET $2
`

type ListGamesParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListGames(ctx context.Context, arg ListGamesParams) ([]VwLibraryGame, error) {
	rows, err := q.db.Query(ctx, listGames, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwLibraryGame
	for rows.Next() {
		var i VwLibraryGame
		if err := rows.Scan(
			&i.ID,
			&i.DisplayTitle,
			&i.Title,
			&i.SanitizedTitle,
			&i.Barcode,
			&i.PlayToWinGameID,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listGamesStatus = `-- name: ListGamesStatus :many
SELECT game_id, game_title, game_barcode, sanitized_title, patron_id, patron_full_name, patron_barcode, transaction_id, checkout_timestamp, checkin_timestamp, checked_out, ptw_game_id
FROM vw_game_status
ORDER BY sanitized_title
LIMIT $1 OFFSET $2
`

type ListGamesStatusParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListGamesStatus(ctx context.Context, arg ListGamesStatusParams) ([]VwGameStatus, error) {
	rows, err := q.db.Query(ctx, listGamesStatus, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwGameStatus
	for rows.Next() {
		var i VwGameStatus
		if err := rows.Scan(
			&i.GameID,
			&i.GameTitle,
			&i.GameBarcode,
			&i.SanitizedTitle,
			&i.PatronID,
			&i.PatronFullName,
			&i.PatronBarcode,
			&i.TransactionID,
			&i.CheckoutTimestamp,
			&i.CheckinTimestamp,
			&i.CheckedOut,
			&i.PtwGameID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPatrons = `-- name: ListPatrons :many
SELECT id, full_name, barcode, created_at
FROM vw_library_patrons
ORDER BY full_name
LIMIT $1 OFFSET $2
`

type ListPatronsParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListPatrons(ctx context.Context, arg ListPatronsParams) ([]VwLibraryPatron, error) {
	rows, err := q.db.Query(ctx, listPatrons, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwLibraryPatron
	for rows.Next() {
		var i VwLibraryPatron
		if err := rows.Scan(
			&i.ID,
			&i.FullName,
			&i.Barcode,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listPlayToWinGameOverviews = `-- name: ListPlayToWinGameOverviews :many
SELECT ptw_game_id, game_id, ptw_group_id, game_title, sanitized_title, created_at, winner_id, winner_name, winner_unique_id
FROM vw_play_to_win_game_overview
LIMIT $1 OFFSET $2
`

type ListPlayToWinGameOverviewsParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListPlayToWinGameOverviews(ctx context.Context, arg ListPlayToWinGameOverviewsParams) ([]VwPlayToWinGameOverview, error) {
	rows, err := q.db.Query(ctx, listPlayToWinGameOverviews, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwPlayToWinGameOverview
	for rows.Next() {
		var i VwPlayToWinGameOverview
		if err := rows.Scan(
			&i.PtwGameID,
			&i.GameID,
			&i.PtwGroupID,
			&i.GameTitle,
			&i.SanitizedTitle,
			&i.CreatedAt,
			&i.WinnerID,
			&i.WinnerName,
			&i.WinnerUniqueID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const resetPlayToWinGameWinners = `-- name: ResetPlayToWinGameWinners :exec
UPDATE play_to_win_games
SET winner_id = NULL
WHERE deletion_reason IS DISTINCT FROM 'claimed'
`

func (q *Queries) ResetPlayToWinGameWinners(ctx context.Context) error {
	_, err := q.db.Exec(ctx, resetPlayToWinGameWinners)
	return err
}

const restorePlayToWinEntry = `-- name: RestorePlayToWinEntry :exec
UPDATE play_to_win_entries
SET deleted_at = NULL,
    deletion_reason = NULL,
    deletion_reason_comment = NULL
WHERE id = $1
`

func (q *Queries) RestorePlayToWinEntry(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, restorePlayToWinEntry, id)
	return err
}

const restorePlayToWinGame = `-- name: RestorePlayToWinGame :exec
UPDATE play_to_win_games
SET deleted_at = NULL,
    deletion_reason = NULL,
    deletion_reason_comment = NULL
WHERE deleted_at IS NOT NULL AND id = $1
`

func (q *Queries) RestorePlayToWinGame(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, restorePlayToWinGame, id)
	return err
}

const restorePlayToWinGameByLibraryGameId = `-- name: RestorePlayToWinGameByLibraryGameId :exec
UPDATE play_to_win_games
SET deleted_at = NULL,
    deletion_reason = NULL,
    deletion_reason_comment = NULL
WHERE deleted_at IS NOT NULL AND game_id = $1
`

func (q *Queries) RestorePlayToWinGameByLibraryGameId(ctx context.Context, gameID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, restorePlayToWinGameByLibraryGameId, gameID)
	return err
}

const restorePlayToWinSession = `-- name: RestorePlayToWinSession :exec
UPDATE play_to_win_sessions
SET deleted_at = NULL,
    deletion_reason = NULL,
    deletion_reason_comment = NULL
WHERE deleted_at IS NOT NULL AND id = $1
`

func (q *Queries) RestorePlayToWinSession(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, restorePlayToWinSession, id)
	return err
}

const searchGameStatus = `-- name: SearchGameStatus :many
SELECT game_id, game_title, game_barcode, sanitized_title, patron_id, patron_full_name, patron_barcode, transaction_id, checkout_timestamp, checkin_timestamp, checked_out, ptw_game_id
FROM vw_game_status
WHERE ($3::boolean IS NULL OR checked_out = $3)
  AND ($4::text IS NULL OR sanitized_title ILIKE $4)
  AND ($5::text IS NULL OR game_barcode ILIKE $5)
ORDER BY sanitized_title
LIMIT $1 OFFSET $2
`

type SearchGameStatusParams struct {
	Limit          int32
	Offset         int32
	CheckedOut     pgtype.Bool
	SanitizedTitle pgtype.Text
	GameBarcode    pgtype.Text
}

func (q *Queries) SearchGameStatus(ctx context.Context, arg SearchGameStatusParams) ([]VwGameStatus, error) {
	rows, err := q.db.Query(ctx, searchGameStatus,
		arg.Limit,
		arg.Offset,
		arg.CheckedOut,
		arg.SanitizedTitle,
		arg.GameBarcode,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwGameStatus
	for rows.Next() {
		var i VwGameStatus
		if err := rows.Scan(
			&i.GameID,
			&i.GameTitle,
			&i.GameBarcode,
			&i.SanitizedTitle,
			&i.PatronID,
			&i.PatronFullName,
			&i.PatronBarcode,
			&i.TransactionID,
			&i.CheckoutTimestamp,
			&i.CheckinTimestamp,
			&i.CheckedOut,
			&i.PtwGameID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchGames = `-- name: SearchGames :many
SELECT id, display_title, title, sanitized_title, barcode, play_to_win_game_id, created_at
FROM vw_library_games
WHERE sanitized_title ILIKE $1
ORDER BY sanitized_title
LIMIT $2 OFFSET $3
`

type SearchGamesParams struct {
	SanitizedTitle string
	Limit          int32
	Offset         int32
}

func (q *Queries) SearchGames(ctx context.Context, arg SearchGamesParams) ([]VwLibraryGame, error) {
	rows, err := q.db.Query(ctx, searchGames, arg.SanitizedTitle, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwLibraryGame
	for rows.Next() {
		var i VwLibraryGame
		if err := rows.Scan(
			&i.ID,
			&i.DisplayTitle,
			&i.Title,
			&i.SanitizedTitle,
			&i.Barcode,
			&i.PlayToWinGameID,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchPatrons = `-- name: SearchPatrons :many
SELECT id, full_name, barcode, created_at
FROM vw_library_patrons
WHERE full_name ILIKE $1
ORDER BY full_name
LIMIT $2 OFFSET $3
`

type SearchPatronsParams struct {
	FullName string
	Limit    int32
	Offset   int32
}

func (q *Queries) SearchPatrons(ctx context.Context, arg SearchPatronsParams) ([]VwLibraryPatron, error) {
	rows, err := q.db.Query(ctx, searchPatrons, arg.FullName, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwLibraryPatron
	for rows.Next() {
		var i VwLibraryPatron
		if err := rows.Scan(
			&i.ID,
			&i.FullName,
			&i.Barcode,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchPlayToWinGameOverviews = `-- name: SearchPlayToWinGameOverviews :many
SELECT ptw_game_id, game_id, ptw_group_id, game_title, sanitized_title, created_at, winner_id, winner_name, winner_unique_id
FROM vw_play_to_win_game_overview
WHERE sanitized_title ILIKE $1
LIMIT $2 OFFSET $3
`

type SearchPlayToWinGameOverviewsParams struct {
	SanitizedTitle string
	Limit          int32
	Offset         int32
}

func (q *Queries) SearchPlayToWinGameOverviews(ctx context.Context, arg SearchPlayToWinGameOverviewsParams) ([]VwPlayToWinGameOverview, error) {
	rows, err := q.db.Query(ctx, searchPlayToWinGameOverviews, arg.SanitizedTitle, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []VwPlayToWinGameOverview
	for rows.Next() {
		var i VwPlayToWinGameOverview
		if err := rows.Scan(
			&i.PtwGameID,
			&i.GameID,
			&i.PtwGroupID,
			&i.GameTitle,
			&i.SanitizedTitle,
			&i.CreatedAt,
			&i.WinnerID,
			&i.WinnerName,
			&i.WinnerUniqueID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const searchTransactionEvents = `-- name: SearchTransactionEvents :many
SELECT transaction_id, game_id, game_title, patron_id, patron_full_name, event_type, event_timestamp, play_to_win_game_id
FROM vw_library_transaction_events
WHERE sanitized_title ILIKE $1 AND patron_full_name ILIKE $2
LIMIT $3 OFFSET $4
`

type SearchTransactionEventsParams struct {
	SanitizedTitle pgtype.Text
	PatronFullName string
	Limit          int32
	Offset         int32
}

type SearchTransactionEventsRow struct {
	TransactionID   pgtype.UUID
	GameID          pgtype.UUID
	GameTitle       string
	PatronID        pgtype.UUID
	PatronFullName  string
	EventType       TransactionEventType
	EventTimestamp  pgtype.Timestamp
	PlayToWinGameID pgtype.UUID
}

func (q *Queries) SearchTransactionEvents(ctx context.Context, arg SearchTransactionEventsParams) ([]SearchTransactionEventsRow, error) {
	rows, err := q.db.Query(ctx, searchTransactionEvents,
		arg.SanitizedTitle,
		arg.PatronFullName,
		arg.Limit,
		arg.Offset,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchTransactionEventsRow
	for rows.Next() {
		var i SearchTransactionEventsRow
		if err := rows.Scan(
			&i.TransactionID,
			&i.GameID,
			&i.GameTitle,
			&i.PatronID,
			&i.PatronFullName,
			&i.EventType,
			&i.EventTimestamp,
			&i.PlayToWinGameID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updatePlayToWinWinner = `-- name: UpdatePlayToWinWinner :exec
UPDATE play_to_win_games
SET winner_id = $2
WHERE id = $1
`

type UpdatePlayToWinWinnerParams struct {
	ID       pgtype.UUID
	WinnerID pgtype.UUID
}

func (q *Queries) UpdatePlayToWinWinner(ctx context.Context, arg UpdatePlayToWinWinnerParams) error {
	_, err := q.db.Exec(ctx, updatePlayToWinWinner, arg.ID, arg.WinnerID)
	return err
}

...
async function searchAndWaitForResults(value: string) {
  ...
  await waitFor(() => expect(apiClient.listPatrons).toHaveBeenCalledWith({ name: value }), {
    timeout: 1000,
  });
}
...
describe('LoanModal', () => {
  ...
  it('Should pre-populate AddPatronModal with the current patron name field value', async () => {
    ...
    await waitFor(() => {
      ...
      const addPatronInput = document.getElementById('addPatronName') as HTMLInputElement | null;
  ...
  it('Should select the new patron and populate the name field after AddPatronModal succeeds', async () => {
    ...
    await waitFor(() => {
      ...
      const patronInput = document.getElementById('patronName') as HTMLInputElement;

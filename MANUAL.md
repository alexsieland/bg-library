...
Front — Loaning a Game (Check Out)

Intro
Keep the process friendly and quick. Most loans follow the same simple steps: find the game, find the patron, assign the copy, set the due date, and confirm the loan.

Steps (follow in order):
1. Open the library web app in your browser. Look for the search box near the top of the screen. It usually has a magnifying glass icon.
2. Find the game:
   - Type part of the title and press Enter, or
   - Use the barcode scanner on the game box if available — scanning support is TBD (work in progress). If scanning is not working, type the barcode manually or use the title search.
3. From the search results, click the game title that matches. If the game has more than one copy, select the correct copy (look at the copy number or barcode).
4. Find the patron:
   - Type the patron’s name, email, or phone in the same search box and press Enter, or
   - If badge scanning is available, scan the patron’s badge/card. Badge scanning is TBD; otherwise search manually.
...
Helpful UI Quick Reference
- Search box: usually at the top. Look for a magnifying glass icon.
- Scan behavior: TBD (work in progress). If scanning isn't available or is not working, type the barcode or search by title.
